"""
    ...
    Both world objects and all of their methods live here:

        - World is the object for accessing world data (e.g. from MDB) and
            working with it.
        - WorldDaemon is the daemon that updates the world data sets in a
            daemon-like fashion

    This module is separate from the api module/app because it has a different
    scope and is kind of a different/separate add-on type utility.

    DO NOT cross the streams anymore than absolutely necessary.
...
# local imports
import api
from api.models import innovations as innovations_models
from api.models import monsters as monster_models
from api.models import expansions as expansions_models
from api.models import settlements as settlements_models
from api.models import survivors as survivors_models
from api.models import campaigns as campaigns_models
from api.models import epithets as epithets_models
import assets as world_assets
import utils
...
class World(object):
    ...
    def refresh_asset(self, asset_key=None, force=False, dump=False):
        ...
        if current_age is None:
            ...
            msg = "Asset '%s' has a current age of %s seconds (max age is %s seconds)."
            self.logger.debug(msg % (asset_key, current_age.seconds, asset_dict["max_age"]))
            do_refresh = True
        if force:
            do_refresh = True
...
#            query.update({ })
    ...
    def recent_sessions(self):
        recent_session_cutoff = datetime.now() - timedelta(hours=utils.settings.get("application", "recent_user_horizon"))
...
class WorldDaemon(object):
    ...
    def __init__(self):
        ...
        self.logger = utils.get_logger(log_name="world_daemon", log_level=utils.settings.get("world","log_level"))
        self.pid_dir = utils.settings.get("application","pid_root_dir")
    ...
    def check_pid_dir(self):
        ...
        pid_dir_owner = pwd.getpwuid(os.stat(self.pid_dir).st_uid).pw_name
    ...
    def start(self):
        ...
        context = daemon.DaemonContext(
            working_directory = (utils.settings.get("api","cwd")),
            detach_process = True,
    ...
    def run(self):
        ...
        self.logger.debug("World Daemon will sleep for %s minutes..." % utils.settings.get("world","refresh_interval"))
