...
def render_documentation(action, render_type):
    response = utils.http_404
    D = docs.DocumentationObject()
    if action == 'get':
        if render_type == 'json':
            j = json.dumps(D.render_as_json(), default=json_util.default)
            response = Response(response=j, status=200, mimetype="application/json")
    elif action == 'get_sections':
        if render_type == 'json':
            j = json.dumps(D.dump_sections(), default=json_util.default)
            response = Response(response=j, status=200, mimetype="application/json")
    return response


# static css and js routes for serving static in dev w/o nginx
...
def serve_image(image_oid):
    ...
    I = utils.gridfs_files.image(image_oid)
